...
// Bilder erstellen
const imgCSS = new Bild("/assets/img/CSS.png", "CSS Logo");
const imgJava = new Bild("/assets/img/Java.png", "Java Logo");
const imgWeb = new Bild("/assets/img/Web.png", "Web Logo");
const imgDB = new Bild("/assets/img/DB.png", "Datenbank Logo");
...
for (let kategorie of alleKategorien) {
  ...
  if (passendeTutorials.length > 0) {
    console.log(`Kategorie: ${kategorie.name}`);
    console.log(`Bild: ${kategorie.bild.name}`);
    for (let tutorial of passendeTutorials) {
      let spracheGross = tutorial.sprache.toUpperCase();
      let datumString = tutorial.datum.toLocaleDateString('de-DE');
      console.log(`   ${tutorial.name}  -  (${spracheGross})  -  ${datumString}`);
      console.log("");
      console.log(`   ${tutorial.beschreibung}`);
      console.log("");
      console.log(`   ${getDauerInStundenUndMinuten(tutorial.dauer)}`);
      console.log("");
      console.log(`   ${tutorial.embededCode} bzw. ${tutorial.url}`);
      for (let kapitel of tutorial.kapitelliste) {
        let minuten = kapitel.dauer.split(':')[1];
        console.log(
          `   ${minuten} Min.   ${kapitel.name}:    ${kapitel.beschreibung}`,
        );
      }
    }
  }
